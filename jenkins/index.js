...
const STYLE = 'color: #eee;font-size: 14px;line-height: 28px;font-family: Helvetica Neue, Helvetica, PingFang SC, Hiragino Sans GB, Microsoft YaHei, Arial, sans-serif'
...
const jenkinsLog = async function (name, nextBuildNumber, jenkins, htmlWebview) {
  ...
  if (next > nextBuildNumber) {
    ...
    log.on('data', function (text) {
      htmlWebview.webview.html += `<div style=${STYLE}>${text}</div>`
    })

    log.on('error', function (err) {
      htmlWebview.webview.html += `<div style=${STYLE}>构建错误${err}</div>`
    })

    log.on('end', function () {
      htmlWebview.webview.html += `<div style=${STYLE}>构建完成</div>`
    })
  } else {
    setTimeout(() => {
      num += 1
      htmlWebview.webview.html = `<div style=${STYLE}>执行中...${num}</div>`
      if (num >= 15) {
        num = 0
        htmlWebview.webview.html = `<div style=${STYLE}>构建失败</div>`
        return
      }
...
const jenkinsInit = context => {
  vscode.commands.registerCommand('jenkins.jenkinsConstruct', async function (res) {
    ...
    if (obj && obj.value) {
      ...
      let picker = await vscode.window.showQuickPick(items, { canPickMany: true, title: obj.description })
  ...
  vscode.commands.registerCommand('jenkins.jenkinsShowLog', async function (res) {
    ...
    htmlWebview.webview.html += `<div style=${STYLE}>${buildLog}</div>`
  })
