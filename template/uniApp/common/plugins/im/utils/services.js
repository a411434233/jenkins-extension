// const RongIMLib = require('@/common/js/RongIMLib-3.0.7-dev.js');

const RongIMLib = require('../lib/RongIMLib-3.0.7.2-dev.min.js');
	...
	let getSender = {
		1: (msg) => {
			msg.sender = {
				...
				type: msg.type,
				avatar1: require('../static/icon1.png'),
				avatar2: require('../static/icon2.png'),
		...
		3: (msg) => {
			msg.sender = {
				...
				avatar: require('../static/icon1.png')
			};
		}
	};
...
Message.sendImage = (params) => {
	...
	console.log('====extraextraextraextra===', extra)
	return sendMessage(type, targetId, {
		type: 'image',
		content,
		imageUri,
...
Conversation.getList = () => {
	return imInstance && imInstance.Conversation.getList().then((list) => {
...
Conversation.clearUnreadCount = (conversation) => {
	console.log('clearUnreadCount', conversation)
	let {
		type,
		targetId
	} = conversation;
...
Status.connect = (user) => {
	// https://docs.rongcloud.cn/v4/views/im/noui/guide/private/setting/listener/web.html?plat=uniapp
	currentUser = user;
	imInstance.watch({
		status: function({
			status
